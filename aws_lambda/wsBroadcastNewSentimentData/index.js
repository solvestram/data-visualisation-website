...
exports.handler = async (event) => {
    ...
    for (let record of event.Records) {
        if (record.eventName === "INSERT") {
            // Create sentiment object in a proper format
            let sentiment_object = {
                TweetId: record.dynamodb.NewImage.TweetId.S,
                TweetTs: record.dynamodb.NewImage.TweetTs.S,
                CurrencySymbol: record.dynamodb.NewImage.CurrencySymbol.S,
                Sentiment: record.dynamodb.NewImage.Sentiment.S,
            };
            
            // Push the object into data array
            data.sentiment_data.data.push(sentiment_object);
        }
    }
